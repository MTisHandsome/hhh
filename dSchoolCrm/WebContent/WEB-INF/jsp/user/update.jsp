...
<body>
	<div class="x-body">
		<form class="" action="../two/uptea.action">
			<input type="hidden" value="${user.userId}" name="userId">
			<label for="L_email" class="layui-form-span"> <span
					class="x-red">注意:修改后请重新选择本校区的车辆及教练</span>
				</label>
			...
			<!-- 
			<input type="hidden" value="${user.teaUserid}" name="teaUserid">
			<input type="hidden" value="${user.carId}" name="carId">
			 -->
			<div class="layui-form-item">
				<label for="L_username" class="layui-form-label"> <span
					class="x-red">*</span>选择校区
				</label>
				<div class="layui-input-inline">

					<!-- <input type="text" id="L_username" name="userSchedule"
						class="layui-input">-->
					<select name="schoolId" class="layui-select" style="width: 130px">
						<c:forEach items="${school}" var="school">
							<option value="${school.schoolId}" ${school.schoolId==user.schoolId?'selected':''}>${school.schoolName}</option>
						</c:forEach>
					</select>
				</div>
			</div>
